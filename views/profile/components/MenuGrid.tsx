...
export const MenuGrid: React.FC<MenuGridProps> = ({ items }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-24 pb-20">
      {sortedCategories.map(category => (
        <section key={category} className="animate-in fade-in slide-in-from-bottom-6 duration-700">
          {/* Subtle Category Header */}
          <div className="flex items-center gap-4 mb-10">
            <h2 className="text-[10px] font-bold text-[#5E6AD2] uppercase tracking-[0.5em] whitespace-nowrap">
              {category}
            </h2>
          ...
          <div className="space-y-12">
            {groupedMenu[category].map((item, idx) => (
              <div 
                key={item.id} 
                className={`group relative transition-all duration-500 ${!item.isAvailable ? 'opacity-20 grayscale pointer-events-none' : 'opacity-100'}`}
              >
                ...
                {/* Sub Row: Description */}
                {item.description && (
                  <p className="text-[13px] text-[#444] leading-relaxed font-light italic mb-3 max-w-[95%] group-hover:text-[#888] transition-colors duration-500">
                    {item.description}
                  </p>
                )}
                ...
                <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
                  ...
                  {(item.availableFrom || item.availableTo) && (
                    <div className="flex items-center gap-1.5 text-[8px] font-bold text-[#1A1A1A] uppercase tracking-[0.2em] group-hover:text-[#5E6AD2]/30 transition-colors">
                      ...
                      <span>{item.availableFrom || '00:00'} — {item.availableTo || '23:59'}</span>
                    </div>
                  )}

                  {!item.isAvailable && (
                    <span className="text-[8px] font-bold text-red-500/10 uppercase tracking-[0.2em]">Private Selection</span>
                  )}
                </div>
